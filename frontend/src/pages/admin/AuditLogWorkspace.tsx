...
export function AuditLogWorkspace() {
  ...
  const handleClear = () => {
    setDraft({ startDate: '', endDate: '', actionTypeId: '', search: '' });
    setFilters({ startDate: '', endDate: '', actionTypeId: '', search: '' });
    setDateError('');
    setPagination((prev) => ({ ...prev, page: 1 }));
  };
    ...
    <div>
      ...
      <div className="bg-white rounded-2xl border border-slate-200 p-4 shadow-sm mb-6">
        <div className="grid gap-4 grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
          {/* Combined Search */}
          <div>
            <label className="block text-sm text-slate-700 mb-1">{t('common.search')}</label>
            <input
              type="text"
              value={draft.search}
              onChange={(e) => setDraft((prev) => ({ ...prev, search: e.target.value }))}
              placeholder={t('admin.audit_log.filters.search_placeholder', '申請番号または実行者名')}
