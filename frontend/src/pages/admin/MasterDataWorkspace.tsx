...
export function MasterDataWorkspace() {
  ...
  const getColumns = (): Column<Record<string, unknown>>[] => {
    ...
    return keys.map((key) => ({
      key,
      header: t(`admin.master_data.columns.${key}`, key.replace(/_/g, ' ').replace(/\b\w/g, (c) => c.toUpperCase())),
      ...(key.endsWith('_name') && {
        render: (val: unknown) => {
          const codeKey = key.replace('_name', '_code');
          const code = (data.find((row) => row[key] === val) as Record<string, unknown>)?.[codeKey] as string | undefined;
          if (code) {
            return t(`admin.master_data.values.${codeKey}.${code}`, val as string);
          }
