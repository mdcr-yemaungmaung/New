...
import type { FrontendNotification } from '../../hooks/useWebSocket';
...
export function Header({ onMenuToggle, notificationCount = 0, notifications = [], onMarkAsRead, onMarkAllAsRead, userRole }: HeaderProps) {
  ...
  const getNotificationText = (noti: FrontendNotification) => {
    const payload = noti.payload;
    if (noti.messageKey) {
      return t(noti.messageKey, noti.messageParams as Record<string, unknown> || {});
    }
