    ...
    try {
      ...
      try {
        await this.notificationService.create(request.applicantUserId, {
          ...
          messageKey: 'notifications.application_approved',
          messageParams: { requestNumber: request.requestNumber },
        });
        ...
        for (const accUser of accountingUsers) {
          await this.notificationService.create(accUser.userId, {
            ...
            messageKey: 'notifications.new_approved_request',
            messageParams: { requestNumber: request.requestNumber },
          });
        }
      } catch (notifError) {
    ...
    try {
      ...
      try {
        await this.notificationService.create(request.applicantUserId, {
          ...
          link: `/applicant/request/${id}`,
          messageKey: 'notifications.application_rejected',
          messageParams: { requestNumber: request.requestNumber },
        });
      } catch (notifError) {
        this.logger.warn(
