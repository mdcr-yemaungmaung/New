      ...
      if (request.managerUserId) {
        try {
          ...
          await this.notificationService.create(request.managerUserId, {
            ...
            messageKey: 'notifications.new_payment_request',
            messageParams: { applicantName, amount: amount.toLocaleString() },
          });
        } catch (err: unknown) {
      ...
      try {
        ...
        for (const approver of approverUsers) {
          await this.notificationService.create(approver.userId, {
            ...
            messageKey: 'notifications.new_request_submitted',
            messageParams: { requestNumber: savedRequest.requestNumber },
          });
        }
      } catch (notifError) {
        this.logger.warn(
